...
const houses=L.layerGroup().addTo(map),rail=L.layerGroup().addTo(map),cem=L.layerGroup().addTo(map),poi=L.layerGroup(),markers={};
...
for(const f of features){
 ...
 const c=f.coords[0];L.circleMarker(c,{radius:f.kind==='kindergarten'?5:3,color:f.kind==='kindergarten'?'#178a59':'#20769b',fillOpacity:.75}).bindPopup(title+'<br><a target="_blank" href="'+esc(f.url)+'">Джерело</a>').addTo(poi);
 }
}
for(const r of rows){
...
L.control.layers({}, {'Будинки й ділянки':houses,'Колії':rail,'Цвинтарі':cem,'Садочки та зупинки':poi},{collapsed:true}).addTo(map);
...
if D.get('landSummary'):report=report.replace('<section id="map-section">','<div class="card"><b>Ділянка під будинок: від 6 соток, до $50 000, житлове призначення та комунікації.</b><p>'+E(D['landSummary'])+'</p></div><section id="map-section">')
if D.get('decisionSummary'):report=report.replace('<section id="map-section">','<div class="card"><b>Пріоритет уточнень і запас коштів</b><p>'+E(D['decisionSummary'])+'</p></div><section id="map-section">')
