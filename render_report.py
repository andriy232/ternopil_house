...
def compact(x):return ' '.join(str(x or '').split())
def kindergartenrow(x):
 ...
 dates='<br><span class="small">Перевірено: '+E(compact(x.get('checkedAt','не встановлено')))+'<br>Оновлення запису: '+E(compact(x.get('recordUpdatedAt','не встановлено')))+'</span>'
 return '<tr><td><b>'+E(compact(x.get('settlement','')))+'</b></td><td>'+E(compact(x.get('name','')))+'<br><span class="small">'+E(compact(x.get('address','')))+'</span></td><td>'+official+dates+'</td><td>'+distance+'<br><span class="small">'+E(compact(x.get('precision','')))+'</span></td><td>'+maps+'</td></tr>'
...
summary_block='<div class="stats"><div class="stat"><b>'+str(len(active))+'</b><span>актуальні кандидати з невизначеностями</span></div><div class="stat"><b>'+str(len(excluded))+'</b><span>відсіяно або поки не включено</span></div><div class="stat"><b>'+str(sum(r.get('qualified_all_criteria') is True for r in rows))+'</b><span>повністю підтверджених відповідностей</span></div></div>'
summary_block+='<div class="notice"><b>«Актуальна сторінка» не означає «підходить за всіма вимогами».</b> '+E(D.get('exclusionsSummary','Прямі виключення та непідтверджені умови наведені в кожному рядку.'))+'</div>'
...
'<section id="kindergartens"><details class="sectionexpander"><summary>Підтверджені садочки в околицях Тернополя <span>'+str(len(confirmed_kindergartens))+'</span></summary><div class="expanderbody"><p class="sectionnote">Офіційний запис закладу й точність його мітки перевіряються окремо.</p>'+kindergartenhtml+'</div></details></section>',
'<section id="summary-end"><h2>Підсумок перевірки</h2>'+summary_block+'<p><a href="changes.md">Зміни проти попереднього звіту</a> · <a href="report.md">Текстовий звіт</a> · <a href="listings.json">Дані JSON</a> · <a href="houses.geojson">Мітки GeoJSON</a></p></section>',
'<footer class="foot">Робочі посилання означають, що сторінку вдалося прочитати під час перевірки. Продаж, ціна, адреса та права на землю остаточно підтверджуються продавцем і документами. Підкладка: OpenFreeMap © OpenMapTiles, дані © OpenStreetMap contributors; Leaflet / MapLibre. Фото належать авторам оголошень і збережені поряд зі звітом. Підкладка карти потребує інтернету; текст, таблиці, мітки та геометрія вбудовані в HTML.</footer></main>']
